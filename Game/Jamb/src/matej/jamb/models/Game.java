...
public class Game {
	...
	public void start() {
		...
		int numOfPlayers = InputChecker.checkInput(1, 5, "number of players");


		List<Player> playerList = new ArrayList<>();
		for (int i = 0; i < numOfPlayers; i++) {
			playerList.add(new Player(InputChecker.checkInput("Player " + (i+1) + " name")));
		}
	...
	public void playTurn(Player player, List<Dice> diceList) {
		...
		while(diceThrows <= Constants.NUM_OF_THROWS) {
			...
			case 3:
				...
				player.getPaper().getRow(rowType).writeDown(diceList, boxType.ordinal());
